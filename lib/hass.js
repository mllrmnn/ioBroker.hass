...
function HASS(options, log) {
    ...
    function query(socket, type, callback) {
        if (socket && typeof socket.send === 'function') {
            const id = currentId++;
            requests[id] = {type: type, cb: callback, ts: Date.now()};
            socket.send(JSON.stringify({
                id: id,
                type: type
            }));
    ...
    this.getEntityRegistry = function (callback) {
        if (!connected) {
            typeof callback === 'function' && callback('not connected');
        } else {
            query(this.socket, 'config/entity_registry/list', callback);
        }
    };

    this.getLabelRegistry = function (callback) {
        if (!connected) {
            typeof callback === 'function' && callback('not connected');
        } else {
            query(this.socket, 'config/label_registry/list', callback);
        }
    };
